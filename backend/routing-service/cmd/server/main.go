...
func main() {
  ...
  if dsn == "" {
    dsn = "host=postgres user=postgres password=postgres dbname=routingdb port=5432 sslmode=disable"
  }
  ...
  pb.RegisterRoutingServiceServer(grpcServer, &ServiceServer{DB: db})
  lis, err := net.Listen("tcp", ":50077")
  if err != nil {
    log.Fatal("Failed to listen:", err)
  }
  go func() {
    log.Println("✅ Routing Service running on port 50077")
