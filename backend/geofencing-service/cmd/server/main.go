...
func main() {
  ...
  if dsn == "" {
    dsn = "host=postgres user=postgres password=postgres dbname=geofencingdb port=5432 sslmode=disable"
  }
  ...
  pb.RegisterGeofencingServiceServer(grpcServer, &ServiceServer{DB: db})
  lis, err := net.Listen("tcp", ":50058")
  if err != nil {
    log.Fatal("Failed to listen:", err)
  }
  go func() {
    log.Println("✅ Geofencing Service running on port 50058")
