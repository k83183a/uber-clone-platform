...
func main() {
  ...
  if dsn == "" {
    dsn = "host=postgres user=postgres password=postgres dbname=ai-conciergedb port=5432 sslmode=disable"
  }
  ...
  pb.RegisterAi-conciergeServiceServer(grpcServer, &ServiceServer{DB: db})
  lis, err := net.Listen("tcp", ":50071")
  if err != nil {
    log.Fatal("Failed to listen:", err)
  }
  go func() {
    log.Println("✅ Ai-concierge Service running on port 50071")
