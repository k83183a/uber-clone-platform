...
func main() {
  ...
  if dsn == "" {
    dsn = "host=postgres user=postgres password=postgres dbname=dispatchdb port=5432 sslmode=disable"
  }
  ...
  pb.RegisterDispatchServiceServer(grpcServer, &ServiceServer{DB: db})
  lis, err := net.Listen("tcp", ":50052")
  if err != nil {
    log.Fatal("Failed to listen:", err)
  }
  go func() {
    log.Println("✅ Dispatch Service running on port 50052")
